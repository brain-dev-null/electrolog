...
def download_past_hour(session: Session, base_url: str) -> DataFrame:
    ...
    responses = [
        session.get(url, params={'h': page, 'f': 'j'})
        for page
        in range(1,3)
    ]
...
def download_past_day(session: Session, base_url: str) -> DataFrame:
    ...
    responses = [
        session.get(url, params={'w': page, 'f': 'j'})
        for page
        in range(1,4)
    ]
...
def download_past_week(session: Session, base_url: str) -> DataFrame:
    ...
    responses = [
        session.get(url, params={'d': page, 'f': 'j'})
        for page
        in range(1,7)
    ]
...
def download_past_year(session: Session, base_url: str) -> DataFrame:
    ...
    responses = [
        session.get(url, params={'m': page, 'f': 'j'})
        for page
        in range(1,13)
    ]
...
def response_to_dataframe(response: Response, value_label: str, date_only=False) -> DataFrame:
    data = response.json()
    initial_time = to_datetime(data['tm'])
    time_offset = timedelta(seconds=data['dt'])

    values = Series(data['val']).dropna()
    times = Series([initial_time + i * time_offset for i in range(len(values))])
