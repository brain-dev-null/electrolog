...
from electrolog.data import download_past_hour, download_past_day, download_past_week, download_past_year
...
class LoginView:
    def __init__(self, root: tk.Tk, default_ip: Optional[str] = "") -> None:
        ...
        ip_entry = tk.Entry(self.login_form, width=20, justify='center', textvariable=self.ip_var)
        ...
        password_entry = tk.Entry(self.login_form, width=20, justify='center', show='*', textvariable=self.pw_var)
        password_entry.grid(row=1, column=1, sticky='e')

        password_label = tk.Label(self.login_form, text='Password: ')
        password_label.grid(row=1, column=0, sticky='e')

        login_button = tk.Button(self.login_form, pady=1, text='Login', relief='groove', command=self.login_button_callback)
...
class ErrorView:
    def __init__(self, message: str) -> None:
        ...
        tk.Button(self.root, text='Ok', padx=10, pady=5, command=self.ok_callback).pack(pady=10)
...
class DataSelectionView:
    ...
    def __init__(self, root: tk.Tk, session: requests.Session, base_url: str) -> None:
        self.session = session
        self.root = root
        self.base_url = base_url
        self.data_selection_form = tk.Frame(root, padx=15, pady=15)

        self.range_selection = ttk.Combobox(self.data_selection_form, values=self.GRANULARITIES, state='readonly')
        self.range_selection.set('Pick a data range')

        export_button = ttk.Button(self.data_selection_form, text='Download data', command=self.export_callback)    
